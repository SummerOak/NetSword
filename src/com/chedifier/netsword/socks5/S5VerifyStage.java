...
public class S5VerifyStage extends AbsS5Stage{
	...
	public Result handle() {
		...
		if(IOUtils.read(getContext().getClientInputStream(),cData, 2) != 2) {
			Log.i(TAG, "socks5 verify failed.");
		...
		if(cData[0] != 0x05) {
			Log.i(TAG, "socks5 verify failed.it is not socks5 protocol");
		...
		if(cData[1] > 0) {
			if(IOUtils.read(getContext().getClientInputStream(), cData,2, cData[1]) != cData[1]) {
				Log.i(TAG, "socks5 verify failed.verify methods wrong");
		...
		if(isLocal()) {
			...
			if(IOUtils.read(getContext().getServerInputStream(),sData, 2) != 2) {
				Log.i(TAG, "socks5 verify failed. proxy server return false data.");
			...
			if(IOUtils.write(getContext().getClientOutputStream(),sData, 2) != 2) {
				Log.i(TAG, "socks5 verify failed. write proxy server return false data.");
